...
int main(int argc, char* argv[])
{
	...
	register int res0,res1,a=-1,b=-1;
	...
	for (i=1; i<SIZE-1; i=i+1) {
		...
		for (j=1; j<SIZE-1; j=j+1 ) {
			 ...
			 * of the derivative.								  */
			register int tmpa = i + a;
			register int tmpa1 = i + a2;
			int tmpa2 = i + a3;
			int tmpb = j + b;
	 ...
	 * the MSE and then the PSNR.									 */
	 k=0;
	int l=SIZE;
       for (j=1; j<SIZE-1; j+=2) {/* allages edw me loop_interchange fernoun megalyterous xronous opote aporiptontai pros to paron*/
		k=k+SIZE;
		l=l+SIZE;
		register int c=k+j,c1=k+(j+1),d=l+j,d1=l+(j+1);
		register int e=output[c] - golden[c],e1=output[c1] - golden[c1],e2=output[d] - golden[d],e3=output[d1] - golden[d1];
                for ( i=1; i<SIZE-1; i+=2 ) {
			t = (output[c] - golden[c])*(output[c] - golden[c]);
                        PSNR += t;
                         t = (output[c1] - golden[c1])*(output[c1] - golden[c1]);
                        PSNR += t;
                        t = (output[d] - golden[d])*(output[d] - golden[d]);
                        PSNR += t;
                        t = (output[d1] - golden[d1])*(output[d1] - golden[d1]);
                        PSNR += t;

                }
        }

	PSNR = PSNR/(double)(SIZE<<12);
