...
int main(int argc, char* argv[])
{
	...
	register int a2=a+1,a3=a+2, a4=a+3;
	register int b2=b+1,b3=b+2, b4=b+3;
	int k=0;
	...
	int l=SIZE;
       for (j=1; j<SIZE-1; j+=2) {/* allages edw me loop_interchange fernoun megalyterous xronous opote aporiptontai pros to paron*/
		k=k+SIZE;
		l=l+SIZE;
		register int e=output[k+j] - golden[k+j],e1=output[k+(j+1)] - golden[k+(j+1)],e2=output[l+j] - golden[l+j],e3=output[l+(j+1)] - golden[l+(j+1)];
