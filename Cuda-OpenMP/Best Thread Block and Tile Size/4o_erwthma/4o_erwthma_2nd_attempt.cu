...
__global__ void convolutionTileRowGPU(float *h_Dst, float *h_Src, int imageW, int imageH, int filterR){
  ...
  if ((bx > 0) && (bx < (imageW / blockDim.x ))) {
    tiled_Src[ty][tx - filterR] = h_Src[iy * imageW + (ix - filterR)];
    tiled_Src[ty][tx + filterR] = h_Src[iy * imageW + (ix + filterR)];
    ...
    tiled_Src[ty][tx] = h_Src[iy * imageW + ix];
    tiled_Src[ty][tx + filterR] = h_Src[iy * imageW + ix + filterR];
    __syncthreads(); //ola ta nhmata tou block exoun oloklhrwsei thn fortwsh ston tiled_Src
    float sum = 0;
    ...
    else if(bx = (imageW / blockDim.x)){

      tiled_Src[ty][tx] = h_Src[iy * imageW + (ix - filterR)];
      tiled_Src[ty][tx + filterR] = h_Src[iy * imageW + ix];
      __syncthreads(); //ola ta nhmata tou block exoun oloklhrwsei thn fortwsh ston tiled_Src
      float sum = 0;
      for (k = -filterR; k <= filterR; k++) {
        int d = tx + k;
        if (d < imageW) {
         sum += tiled_Src[ty][d + filterR] * Filter_in_constant_mem[filterR - k];
         __syncthreads(); //gia na eksasfalisei oti ta nhmata tou block exoun teliwsei me ton tiled_Src
         h_Dst[iy * imageW + ix] = sum;
          __syncthreads();
        }
...
__global__ void convolutionTileColumnGPU(float *h_Dst, float *h_Src, int imageW, int imageH, int filterR){
  ...
  if ((by > 0) && (by < (imageW / blockDim.y))) {
    tiled_Src[ty - filterR][tx] = h_Src[(iy - filterR) * imageW + ix];
    tiled_Src[ty + filterR][tx] = h_Src[(iy + filterR) * imageW + ix ];
    ...
    else if(by = (imageH / blockDim.y)){

      tiled_Src[ty][tx] = h_Src[(iy - filterR) * imageW + ix];
      tiled_Src[ty + filterR][tx] = h_Src[iy * imageW + ix];
      __syncthreads(); //ola ta nhmata tou block exoun oloklhrwsei thn fortwsh ston tiled_Src
      float sum = 0;
      for (k = -filterR; k <= filterR; k++) {
        int d = ty + k;
        if (d < imageH) {
         sum += tiled_Src[d + filterR][tx] * Filter_in_constant_mem[filterR - k];
         __syncthreads(); //gia na eksasfalisei oti ta nhmata tou block exoun teliwsei me ton tiled_Src
         h_Dst[iy * imageW + ix] = sum;
          __syncthreads();
        }
