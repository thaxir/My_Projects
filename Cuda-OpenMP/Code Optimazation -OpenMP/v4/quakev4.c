...
/* Case info */
  ...
  printf("  Orientation:  strike: %f\n", Src.strike);
  printf("                   dip: %f\n", Src.dip);
  printf("                  rake: %f\n", Src.rake);
  printf("           dislocation: %f cm\n", Src.fault);
  printf("Hypocenter: (%f, %f, %f) Km\n", 
	  Src.xyz[0], Src.xyz[1], Src.xyz[2]);
  printf("Excitation characteristics\n");
  printf("     Time step: %f sec\n", Exc.dt);
...
/*        for the node closest to the epicenter */
  ...
  #pragma omp parallel private(my_cpu_id)  
  {
    ...
    #pragma omp for private(d1,d2,c0) schedule(guided) nowait//periergws auto to nowait dn alloiwnei ta apotelesmata
      for (i = 0; i < ARCHnodes; i++) {
        c0[0] = ARCHcoord[i][0];
        c0[1] = ARCHcoord[i][1];
        c0[2] = ARCHcoord[i][2];
...
#pragma omp parallel for private(j)  
    ...
    time = iter * Exc.dt;
#pragma omp parallel  	//vgazw auto kai vazw parallel sto katw katevazei to 
{
#pragma omp for private(j) schedule(guided) nowait
  ...
  free (w1);
  printf("%d nodes %d elems %d timesteps\n", 
...
#pragma omp parallel private(my_cpu_id) 
  ...
  my_cpu_id=omp_get_thread_num();
//#pragma omp for private(Anext,Alast,sum0,sum1,sum2,col) schedule(guided) 
#pragma omp for schedule(guided) reduction(+:sum0,sum1,sum2)
  for (i = 0; i < nodes; i++) {
    Anext = Aindex[i];
    Alast = Aindex[i + 1];
 ...
 #pragma omp for private(j) nowait//me schedule guided kai static anevenei o xronos peripou 2 sec
  for (i = 0; i < nodes; i++) {
    for (j = 0; j < numthreads; j++) {
      if (w2[j][i]) {
	w[i][0] += w1[j][i].first;
