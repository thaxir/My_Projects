...
double sobel(unsigned char *input, unsigned char *output, unsigned char *golden)
{
	...
	for (i=1; i<SIZE-1; i+=1) {
		for (j=1; j<SIZE-1; j+=4 ) {
			...
			res0 = 0;/* antikatastash tou convolution2D(j, i, input, horiz_operator) */
                        res0 += input[(i + a)*SIZE + (j+2) + b] * horiz_operator[a+1][b+1];
                        res0 += input[(i + a)*SIZE + (j+2) + (b+1)] * horiz_operator[a+1][b+2];
                        res0 += input[(i + a)*SIZE + (j+2) + (b+2)] * horiz_operator[a+1][b+3];

                        res0 += input[(i + (a+1))*SIZE + (j+2) + b] * horiz_operator[a+2][b+1];
                        res0 += input[(i + (a+1))*SIZE + (j+2) + (b+1)] * horiz_operator[a+2][b+2];
                        res0 += input[(i + (a+1))*SIZE + (j+2) + (b+2)] * horiz_operator[a+2][b+3];

                        res0 += input[(i + (a+2))*SIZE + (j+2) + b] * horiz_operator[a+3][b+1];
                        res0 += input[(i + (a+2))*SIZE + (j+2) + (b+1)] * horiz_operator[a+3][b+2];
                        res0 += input[(i + (a+2))*SIZE + (j+2) + (b+2)] * horiz_operator[a+3][b+3];

                        res0 = pow(res0, 2);

                        res1 = 0;/* antikatastash tou convolution2D(j, i, input,vert_operation) */
                        res1 += input[(i + a)*SIZE + (j+2) + b] * vert_operator[a+1][b+1]; 
                        res1 += input[(i + a)*SIZE + (j+2) + (b+1)] * vert_operator[a+1][b+2];
                        res1 += input[(i + a)*SIZE + (j+2) + (b+2)] * vert_operator[a+1][b+3];

                        res1 += input[(i + (a+1))*SIZE + (j+2) + b] * vert_operator[a+2][b+1];
                        res1 += input[(i + (a+1))*SIZE + (j+2) + (b+1)] * vert_operator[a+2][b+2];
                        res1 += input[(i + (a+1))*SIZE + (j+2) + (b+2)] * vert_operator[a+2][b+3];

                        res1 += input[(i + (a+2))*SIZE + (j+2) + b] * vert_operator[a+3][b+1];
                        res1 += input[(i + (a+2))*SIZE + (j+2) + (b+1)] * vert_operator[a+3][b+2];
                        res1 += input[(i + (a+2))*SIZE + (j+2) + (b+2)] * vert_operator[a+3][b+3];
                        res1 = pow(res1, 2);
                        p = res0 + res1;
                        res = (int)sqrt(p);
                        /* If the resulting value is greater than 255, clip it *
			...
			res0 = 0;/* antikatastash tou convolution2D(j, i, input, horiz_operator) */
                        res0 += input[(i + a)*SIZE + (j+3) + b] * horiz_operator[a+1][b+1];
                        res0 += input[(i + a)*SIZE + (j+3) + (b+1)] * horiz_operator[a+1][b+2];
                        res0 += input[(i + a)*SIZE + (j+3) + (b+2)] * horiz_operator[a+1][b+3];

                        res0 += input[(i + (a+1))*SIZE + (j+3) + b] * horiz_operator[a+2][b+1];
                        res0 += input[(i + (a+1))*SIZE + (j+3) + (b+1)] * horiz_operator[a+2][b+2];
                        res0 += input[(i + (a+1))*SIZE + (j+3) + (b+2)] * horiz_operator[a+2][b+3];

                        res0 += input[(i + (a+2))*SIZE + (j+3) + b] * horiz_operator[a+3][b+1];
                        res0 += input[(i + (a+2))*SIZE + (j+3) + (b+1)] * horiz_operator[a+3][b+2];
                        res0 += input[(i + (a+2))*SIZE + (j+3) + (b+2)] * horiz_operator[a+3][b+3];

                        res0 = pow(res0, 2);

                        res1 = 0;/* antikatastash tou convolution2D(j, i, input,vert_operation) */
                        res1 += input[(i + a)*SIZE + (j+3) + b] * vert_operator[a+1][b+1]; 
                        res1 += input[(i + a)*SIZE + (j+3) + (b+1)] * vert_operator[a+1][b+2];
                        res1 += input[(i + a)*SIZE + (j+3) + (b+2)] * vert_operator[a+1][b+3];

                        res1 += input[(i + (a+1))*SIZE + (j+3) + b] * vert_operator[a+2][b+1];
                        res1 += input[(i + (a+1))*SIZE + (j+3) + (b+1)] * vert_operator[a+2][b+2];
                        res1 += input[(i + (a+1))*SIZE + (j+3) + (b+2)] * vert_operator[a+2][b+3];

                        res1 += input[(i + (a+2))*SIZE + (j+3) + b] * vert_operator[a+3][b+1];
                        res1 += input[(i + (a+2))*SIZE + (j+3) + (b+1)] * vert_operator[a+3][b+2];
                        res1 += input[(i + (a+2))*SIZE + (j+3) + (b+2)] * vert_operator[a+3][b+3];
                        res1 = pow(res1, 2);
                        p = res0 + res1;
                        res = (int)sqrt(p);
                        /* If the resulting value is greater than 255, clip it *
	...
	for (i=1; i<SIZE-1; i++) {/* allages edw me loop_interchange fernoun megalyterous xronous opote aporiptontai pros to paron*/
		for ( j=1; j<SIZE-1; j++ ) {
