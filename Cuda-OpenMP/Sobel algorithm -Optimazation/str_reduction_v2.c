...
int convolution2D(int posy, int posx, const unsigned char *input, char operator[][3]) {
	...
	int j2=j+1,j3=j+2,j4=j+3;
	int tmp1=posy+i,tmp2=posy+i2,tmp3=posy+i3;
	int tmp4=posx+j,tmp5=posx+j2,tmp6=posx+j3;
		res += input[(tmp1)*SIZE + tmp4] * operator[i2][j2];
		res += input[(tmp1)*SIZE + tmp5] * operator[i2][j3];
		res += input[(tmp1)*SIZE + tmp6] * operator[i2][j4];

		res += input[(tmp2)*SIZE + tmp4] * operator[i3][j2];
		res += input[(tmp2)*SIZE + tmp5] * operator[i3][j3];
		res += input[(tmp2)*SIZE + tmp6] * operator[i3][j4];

		res += input[(tmp3)*SIZE + tmp4] * operator[i4][j2];
		res += input[(tmp3)*SIZE + tmp5] * operator[i4][j3];
		res += input[(tmp3)*SIZE + tmp6] * operator[i4][j4];
       ...
       for (i=1; i<SIZE-1; i+=2) {/* allages edw me loop_interchange fernoun megalyterous xronous opote aporiptontai pros to paron*/
		...
		l=l+SIZE;
                for ( j=1; j<SIZE-1; j+=2 ) {
                        t = pow((output[k+j] - golden[k+j]),2);
                        PSNR += t;
                         t = pow((output[k+(j+1)] - golden[k+(j+1)]),2);
                        PSNR += t;
                        t = pow((output[l+j] - golden[l+j]),2);
                        PSNR += t;
                        t = pow((output[l+(j+1)] - golden[l+(j+1)]),2);
                        PSNR += t;
