...
double sobel(unsigned char *input, unsigned char *output, unsigned char *golden)
{
	...
	register int res0,res1,a,b,a2,a3,a4,b2,b3,b4;
	a=-1;b=-1;
	...
	for (i=1; i<SIZE-1; i=i+1) {
		...
		for (j=1; j<SIZE-1; j=j+1 ) {
			 ...
			 * of the derivative.								  */
			register int tmpa = i + a, tmpa1 = i + a2, tmpa2 = i + a3,tmpb = j + b,tmpb1 = j + b2/* j+b+1*/,tmpb2 = j + b3;/*j +b+2*/
	 ...
	 * the MSE and then the PSNR.									 */
	 k=0;
	int l=SIZE;
       for (i=1; i<SIZE-1; i=i+2) {/* allages edw me loop_interchange fernoun megalyterous xronous opote aporiptontai pros to paron*/
		k=k+SIZE;
		l=l+SIZE;
