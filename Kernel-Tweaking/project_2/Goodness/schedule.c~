...
struct task_struct * best_goodn(){
	
	unsigned long long best;
	struct task_struct *pick_this;	//gia na dialeksw to process me to kalytero burst pou tha vrw
	struct task_struct *head;

	unsigned long long min_exp_burst;
	unsigned long long max_sleeping;
	long double best_goodness;




	
	for (head = rq->head->next, min_exp_burst = head->exp_burst, max_sleeping = 0; head != rq->head; head = head->next) {
	  
		head->goodness = 0;				//arxikopoiw thn goodness kathe task
	  
		if (head->exp_burst < min_exp_burst){
	    		min_exp_burst = head->exp_burst;		//edw ypologizw to min_exp_burst
	  	}

		head->sleeping = sched_clock() - head->last;	//edw ypologizw to max sleeping
	  	if (head->sleeping > max_sleeping){
	    		max_sleeping = head->sleeping;
		}
	} 
	
	for (head = rq->head->next ; head != rq->head; head = head->next) {
	  head->goodness = (double) (((1+head->exp_burst)/(min_exp_burst + 1))*((1+max_sleeping)/(1+head->sleeping)));	//ypologismos goodness
	}

	for (head = rq->head->next, pick_this = head, best_goodness = head->goodness; head != rq->head; head = head->next) {	//ypologismos tou best goodness
	
	  if(head->goodness < best_goodness) {
	    best_goodness = head->goodness;
	    pick_this=head;
	  }
	}
